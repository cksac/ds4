...
impl ModelViews {
    ...
    pub fn map_model_range(
        ...
        if max_buffer == 0 || max_buffer <= overlap { anyhow::bail!("Metal maxBufferLength too small for model views"); }
        ...
        while off < mapped_model_size {
            ...
            self.wrap_count += 1; self.wrap_bytes += view_bytes;
    ...
    pub fn wrap_model_range(&self, model_map: *const std::ffi::c_void, model_size: u64, offset: u64, len: u64) -> Result<(Retained<AnyObject>, u64)> {
        if model_size == 0 || offset > model_size || len > model_size - offset { anyhow::bail!("model range outside mapped model"); }
        let end = offset + len;
        for view in &self.views {
            if view.model_map != model_map || view.model_size != model_size { continue; }
            if offset >= view.model_offset && end <= view.model_offset + view.bytes {
                return Ok((view.buffer.clone(), offset - view.model_offset));
            }
        }
...
/// A lazily-allocated scratch buffer. Uses interior mutability (RefCell)
/// so that `ensure` can be called through a shared reference.
pub struct ScratchBuffer {
    pub buffer: RefCell<Option<Retained<AnyObject>>>,
    pub capacity: RefCell<usize>,
    label: &'static str,
}

impl ScratchBuffer {
    pub fn new(label: &'static str) -> Self {
        Self { buffer: RefCell::new(None), capacity: RefCell::new(0), label }
    }
    pub fn ensure(&self, device: &AnyObject, bytes: usize) -> Result<()> {
        let cap = *self.capacity.borrow();
        if cap >= bytes && self.buffer.borrow().is_some() { return Ok(()); }
